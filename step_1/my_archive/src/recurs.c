...
** Last update Sun Jan 10 03:18:32 2016 marc brout
*/
...
char		concate_strs(t_arg *arg, struct dirent *file, t_file *par)
{
  ...
  size = strlen(par->path) + strlen(file->d_name) + 2;
  if ((str = malloc(size + 1)) == NULL)
    return (1);
  str[size] = 0;
  if (snprintf(str, size, "%s%c%s", par->path, '/', file->d_name) < 0)
    return (1);
  size = strlen(par->archpath) + strlen(file->d_name) + 1;
  if ((str2 = malloc(size + 1)) == NULL ||
